...
def Modes(usp, vs, np, mouse, hp):
    if usp == False and vs == False and np is None and hp == False and mouse == False:
        return 'normal_mode'

    elif usp == True and vs == False and np is None and hp == False and mouse == False:
        return 'usp_mode'

    elif usp == True and vs == False and np is None and hp == False and mouse == True:
        return 'usp_w_mouse_mode'

    elif usp == False and vs == True and np is None and hp == False and mouse == False:
        return 'vs_mode'

    elif usp == False and vs == False and np is not None and hp == False and mouse == False:
        return 'np_mode'

    elif usp == False and vs == False and np is not None and hp == False and mouse == True:
        return 'pn_w_mouse_mode'

    elif usp == False and vs == False and np is None and hp == True and mouse == False:
        return 'hp_mode'
...
def menu():
    ...
    #   print("- TRANSPARENCY -" + "    -> PRESS " + Fore.RED + "'l'" + Fore.RESET)
...
def main():
    ...
    parser.add_argument('-hp', '--hand_painting', action='store_true', help='Using finger as pencil')
    ...
    while True:
        ...
        mode = Modes(args['use_shake_prevention'], args['video_stream'], args['numbers_paint'], mouse_toggle, args['hand_painting'])

        # Frame copies for overlays and new windows
        frame_for_segmentation = frame.copy()
        frame_for_hand_paiting = frame.copy()
        ...
        if mode == 'hp_mode':
            rgb_hand = cv2.cvtColor(frame_for_hand_paiting, cv2.COLOR_BGR2RGB)
        ...
        if mode == 'hp_mode':
            ...
            cv2.line(img = whiteboard_hand, pt1 = previous_point_hp, pt2 = centroid_finger, color = painting_color, thickness = radius)
            previous_point_hp = centroid_finger
        ...
        if args['video_stream']:
            ...
            cv2.namedWindow('Hand Window', cv2.WINDOW_NORMAL)
            cv2.imshow('Hand Window', rgb_hand)
            
            # Plotting the whiteboard
            cv2.namedWindow('Hand WhiteBoard', cv2.WINDOW_NORMAL)
            cv2.imshow('Hand WhiteBoard', whiteboard_hand)
